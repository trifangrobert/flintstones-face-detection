...
def eval_detections(detections, scores, file_names, ground_truth_path):
    ...
    ground_truth_detections = np.array(ground_truth_file[:, 1:5], np.int64)
    ...
    print('All faces: average precision %.3f' % average_precision)
    plt.savefig(f'./results/{result_index}/precizie_medie_all_faces.png')
    # plt.show()
...
if __name__ == "__main__":
    ...
    parser.add_argument("-r", "--result_index", type=int, required=True)
    ...
    solution_path_root = f"./results/{result_index}/"
    # ground_truth_path_root = "../val/"
    ground_truth_path_root = "../val_train/"
    ...
    ground_truth_path = ground_truth_path_root + f"{actor}_annotations.txt"
    # print(solution_path)
